...
.controller('RegisterCtrl', ['$scope', '$firebaseAuth', '$location','$http','CommonProp','$rootScope', function($scope, $firebaseAuth, $location,$http,CommonProp,$rootScope){
	$rootScope.username = CommonProp.getUser();

	//console.log("$rootScope.userLoginToken",$rootScope.userLoginToken);
	// var user = firebase.auth().currentUser;

	// user.getToken().then(function(accessToken) {
	// 	//console.log(accessToken)
	// 	$scope.getToken(accessToken);
	// });
	...
	console.log(firstName+"    "+lastName+"    "+company+"    "+legalOrTradingName+"    "+lineOfBusiness+"    "+organizationDesc+"    "+gstRegistrationNum+"    "+website+"    "+address
		...
		//Call the services
		// $http.post('http://ec2-54-236-19-206.compute-1.amazonaws.com/billlive/company/signup', JSON.stringify(data)).then(function (response) {
		// if (response.data)
		...
		$scope.samples = ["Andhra Pradesh","Arunachal Pradesh","Assam","Bihar","Chhattisgarh","Dadra and Nagar Haveli","Daman and Diu","Delhi","Goa","Gujarat","Haryana","Himachal Pradesh","Jammu and Kashmir","Jharkhand","Karnataka",
